...
| Test | Kind | Proves |
|------|------|--------|
| `test_protocol.cpp` | native C++ (g++/clang, `-std=c++17`) | The shipping `petkit_protocol.h` — CRC, frame build/validate, status parsing — is correct, rejects corrupted/truncated frames, and `build_frame` reproduces **captured** get-status, open/close-door (`0x1E`), stock dispense (`00 02 01 50`), and the 19-byte `0x0D` boot config byte-for-byte. |
| `test_framer.cpp` | native C++ | The shipping `petkit_framer.h` — the RX `FrameAssembler` resyncs correctly (stray/triple `0xAA`, bogus length, split and back-to-back frames), and the `PayloadQueue` keeps each entry's own payload in FIFO order and respects capacity. |
